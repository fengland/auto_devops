...
# 当账号余额不足20的时候进行报区
# 使用了bc对浮点数进行比较
# 读取文件直接写文件名会导致cron任务执行的时候找不到account.info,程序执行不正确
...
function search_alert(){
  ...
  mail -s "POST账户余额不足告警"  -c "devops@npool.com,nicholas.liu@npool.com,tanghong@npool.com"  wangxufeng@npool.com  <${ALERT_MESSAGE}  >>/tmp/mailx.log 2>&1  
